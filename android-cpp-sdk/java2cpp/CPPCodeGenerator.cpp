...
std::ostream& CPPCodeGenerator::write_header(std::ostream &os) const
{
	...
	os <<
		"  author: Zoran Angelov, mailto://baldzar@gmail.com" << ::std::endl;
	os <<
...
std::ostream& CPPCodeGenerator::write_class(std::ostream &os, std::size_t indent_pos, shared_ptr<model::Class> const &clazz) const
{
	...
	indent(os,indent_pos+1) <<
		": public object<" << clazz->get_name() << ">" << std::endl;
	indent(os,indent_pos) <<
		"{" << std::endl;
	indent(os,indent_pos) <<
	...
	indent(os,indent_pos+1) << ": object<" << clazz->get_name() << ">(jobj)" << std::endl;
				...
				if(pMethod->is_constructor())
				{
					...
					for(std::size_t arg=0;arg<pMethod->get_argument_types().size();++arg)
					{
						...
						if(methodArgType->is_intrinsic())
							...
							os << "local_ref< " << methodArgType->get_cxx_type() << " > const&";
					}
					os << ");" << std::endl;
				}
				else
			...
			if(pMethod->test_access_flag(model::ClassMember::kPublic))
			{
				if(pMethod->is_constructor())
				{
					...
					for(std::size_t arg=0;arg<pMethod->get_argument_types().size();++arg)
					{
						...
						format argNameFmt("a%1%");
						argNameFmt % arg;
						os << argNameFmt.str();
					}
					os << ")" << std::endl;

					indent(os,0) << ": object<" << clazz->get_cxx_class_name() << ">(" << std::endl;
					...
					if(clazz->get_fields_count())
					{
						for(std::size_t f=0;f<clazz->get_fields_count();++f)
						{
							...
							if(pField->test_access_flag(model::ClassMember::kPublic))
							{
								if(!pField->test_access_flag(model::ClassMember::kStatic))
								{
									indent(os,0) << ", " << pField->get_name() << "(get_jobject())" << std::endl;
								}
							}
						}
					}
				...
				else
				{
					...
					if(pMethod->test_access_flag(model::ClassMember::kStatic))
					{
						os << "call_static_method<" << std::endl;
					}
					else
					{
						os << "call_method<" << std::endl;
					}
...
std::string CPPCodeGenerator::transform_method_name(std::string const &original) const
{
	if(	algorithm::is_equal()(original,"delete") ||
		algorithm::is_equal()(original,"and") ||
		algorithm::is_equal()(original,"or") ||
		algorithm::is_equal()(original,"xor") ||
		algorithm::is_equal()(original,"union") ||
		algorithm::is_equal()(original,"register") ||
		algorithm::is_equal()(original,"array") ||
		algorithm::is_equal()(original,"object") ||
		algorithm::is_equal()(original,"struct") ||
		algorithm::is_equal()(original,"operator") ||
		algorithm::is_equal()(original,"mutable") ||
		algorithm::is_equal()(original,"volatile") ||
		algorithm::is_equal()(original,"volatile") )
	{
		return original+"_";
	}
