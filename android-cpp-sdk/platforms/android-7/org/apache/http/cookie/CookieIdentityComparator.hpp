/*================================================================================
  code generated by: java2cpp
  class: org.apache.http.cookie.CookieIdentityComparator
================================================================================*/


#ifndef J2CPP_INCLUDE_IMPLEMENTATION

#ifndef J2CPP_ORG_APACHE_HTTP_COOKIE_COOKIEIDENTITYCOMPARATOR_HPP_DECL
#define J2CPP_ORG_APACHE_HTTP_COOKIE_COOKIEIDENTITYCOMPARATOR_HPP_DECL


namespace j2cpp { namespace java { namespace lang { class Object; } } }
namespace j2cpp { namespace org { namespace apache { namespace http { namespace cookie { class Cookie; } } } } }


#include <java/lang/Object.hpp>
#include <org/apache/http/cookie/Cookie.hpp>


namespace j2cpp {

namespace org { namespace apache { namespace http { namespace cookie {

	class CookieIdentityComparator;
	class CookieIdentityComparator
		: public cpp_object<CookieIdentityComparator>
	{
	public:

		J2CPP_DECLARE_CLASS

		J2CPP_DECLARE_METHOD(0)
		J2CPP_DECLARE_METHOD(1)
		J2CPP_DECLARE_METHOD(2)

		CookieIdentityComparator(jobject jobj)
		: cpp_object<CookieIdentityComparator>(jobj)
		{
		}

		cpp_int compare(local_ref< org::apache::http::cookie::Cookie > const&, local_ref< org::apache::http::cookie::Cookie > const&);
		cpp_int compare(local_ref< java::lang::Object > const&, local_ref< java::lang::Object > const&);
	}; //class CookieIdentityComparator

} //namespace cookie
} //namespace http
} //namespace apache
} //namespace org


} //namespace j2cpp

#endif //J2CPP_ORG_APACHE_HTTP_COOKIE_COOKIEIDENTITYCOMPARATOR_HPP_DECL

#else //J2CPP_INCLUDE_IMPLEMENTATION

#ifndef J2CPP_ORG_APACHE_HTTP_COOKIE_COOKIEIDENTITYCOMPARATOR_HPP_IMPL
#define J2CPP_ORG_APACHE_HTTP_COOKIE_COOKIEIDENTITYCOMPARATOR_HPP_IMPL

namespace j2cpp {


template <>
local_ref< org::apache::http::cookie::CookieIdentityComparator > create< org::apache::http::cookie::CookieIdentityComparator>()
{
	return local_ref< org::apache::http::cookie::CookieIdentityComparator >(
		environment::get().get_jenv()->NewObject(
			get_class<org::apache::http::cookie::CookieIdentityComparator::J2CPP_CLASS_NAME>(),
			get_method_id<org::apache::http::cookie::CookieIdentityComparator::J2CPP_CLASS_NAME, org::apache::http::cookie::CookieIdentityComparator::J2CPP_METHOD_NAME(0), org::apache::http::cookie::CookieIdentityComparator::J2CPP_METHOD_SIGNATURE(0), false>()
		)
	);
}

cpp_int org::apache::http::cookie::CookieIdentityComparator::compare(local_ref< org::apache::http::cookie::Cookie > const &a0, local_ref< org::apache::http::cookie::Cookie > const &a1)
{
	return cpp_int(
		environment::get().get_jenv()->CallIntMethod(
			get_jtype(),
			get_method_id<J2CPP_CLASS_NAME, J2CPP_METHOD_NAME(1), J2CPP_METHOD_SIGNATURE(1), false>(),
			a0.get_jtype(), a1.get_jtype()
		)
	);
}

cpp_int org::apache::http::cookie::CookieIdentityComparator::compare(local_ref< java::lang::Object > const &a0, local_ref< java::lang::Object > const &a1)
{
	return cpp_int(
		environment::get().get_jenv()->CallIntMethod(
			get_jtype(),
			get_method_id<J2CPP_CLASS_NAME, J2CPP_METHOD_NAME(2), J2CPP_METHOD_SIGNATURE(2), false>(),
			a0.get_jtype(), a1.get_jtype()
		)
	);
}


J2CPP_DEFINE_CLASS(org::apache::http::cookie::CookieIdentityComparator,"org/apache/http/cookie/CookieIdentityComparator")
J2CPP_DEFINE_METHOD(org::apache::http::cookie::CookieIdentityComparator,0,"<init>","()V")
J2CPP_DEFINE_METHOD(org::apache::http::cookie::CookieIdentityComparator,1,"compare","(Lorg/apache/http/cookie/Cookie;Lorg/apache/http/cookie/Cookie;)I")
J2CPP_DEFINE_METHOD(org::apache::http::cookie::CookieIdentityComparator,2,"compare","(Ljava/lang/Object;Ljava/lang/Object;)I")

} //namespace j2cpp

#endif //J2CPP_ORG_APACHE_HTTP_COOKIE_COOKIEIDENTITYCOMPARATOR_HPP_IMPL

#endif //J2CPP_INCLUDE_IMPLEMENTATION
