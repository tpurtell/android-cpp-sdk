/*================================================================================
  code generated by: java2cpp
  class: android.graphics.PathEffect
================================================================================*/


#ifndef J2CPP_INCLUDE_IMPLEMENTATION

#ifndef J2CPP_ANDROID_GRAPHICS_PATHEFFECT_HPP_DECL
#define J2CPP_ANDROID_GRAPHICS_PATHEFFECT_HPP_DECL






namespace j2cpp {

namespace android { namespace graphics {

	class PathEffect;
	class PathEffect
		: public cpp_object<PathEffect>
	{
	public:

		J2CPP_DECLARE_CLASS

		J2CPP_DECLARE_METHOD(0)
		J2CPP_DECLARE_METHOD(1)

		PathEffect(jobject jobj)
		: cpp_object<PathEffect>(jobj)
		{
		}

	}; //class PathEffect

} //namespace graphics
} //namespace android


} //namespace j2cpp

#endif //J2CPP_ANDROID_GRAPHICS_PATHEFFECT_HPP_DECL

#else //J2CPP_INCLUDE_IMPLEMENTATION

#ifndef J2CPP_ANDROID_GRAPHICS_PATHEFFECT_HPP_IMPL
#define J2CPP_ANDROID_GRAPHICS_PATHEFFECT_HPP_IMPL

namespace j2cpp {


template <>
local_ref< android::graphics::PathEffect > create< android::graphics::PathEffect>()
{
	return local_ref< android::graphics::PathEffect >(
		environment::get().get_jenv()->NewObject(
			get_class<android::graphics::PathEffect::J2CPP_CLASS_NAME>(),
			get_method_id<android::graphics::PathEffect::J2CPP_CLASS_NAME, android::graphics::PathEffect::J2CPP_METHOD_NAME(0), android::graphics::PathEffect::J2CPP_METHOD_SIGNATURE(0), false>()
		)
	);
}



J2CPP_DEFINE_CLASS(android::graphics::PathEffect,"android/graphics/PathEffect")
J2CPP_DEFINE_METHOD(android::graphics::PathEffect,0,"<init>","()V")
J2CPP_DEFINE_METHOD(android::graphics::PathEffect,1,"finalize","()V")

} //namespace j2cpp

#endif //J2CPP_ANDROID_GRAPHICS_PATHEFFECT_HPP_IMPL

#endif //J2CPP_INCLUDE_IMPLEMENTATION
